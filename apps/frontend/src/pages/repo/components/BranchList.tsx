...
export const BranchList: React.FC<BranchListProps> = ({ branches, currentBranch, onSwitchBranch, previewCount = 5 }) => {
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-sm font-semibold text-[#e8e8e8] uppercase tracking-wider">Branches</h2>
      </div>
      ...
      {branches.length === 0 ? (
        ...
        <div className="space-y-1">
          {previewBranches.map((branch) => (
            <BranchRow key={branch} branch={branch} currentBranch={currentBranch} />
          ))}
      ...
      {isOpen && <BranchDialog branches={branches} currentBranch={currentBranch} onClose={() => setIsOpen(false)} onSwitchBranch={onSwitchBranch} />}
    </div>
  );
};
...
const BranchRow: React.FC<{
  ...
  <div
    className={`p-2.5 rounded border transition-colors ${
      branch === currentBranch ? "bg-app-accent/10 border-app-accent" : "bg-transparent border-transparent hover:bg-[#353535] hover:border-[#3d3d3d]"
    }`}
  >
...
const BranchDialog: React.FC<BranchDialogProps> = ({ branches, currentBranch, onClose, onSwitchBranch }) => {
  return (
    <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-app-bg w-full max-w-md rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="p-3 max-h-[60vh] overflow-y-auto space-y-2">
          {branches.map((branch) => (
            <div key={branch} className="flex items-center justify-between p-2 rounded hover:bg-[#353535]">
